...
public class Huffman {
    public static void main(String args[]) /*throws Exception*/ {
        ...
        while (PQ.size() > 1) { //while there are two or more Trees left in the forest
            ...
            Tree tree = new Tree(), x = PQ.poll(), y = PQ.poll();
            Node node = new Node();

            tree.root = node;
            tree.frequency = x.frequency + y.frequency;
            ...
            node.smallestLetter = x.root.smallestLetter;

            node.leftChild = l;
            node.rightChild = r;*/

            tree.root.leftChild = x.root;
            tree.root.rightChild = y.root;

            if((int)x.root.smallestLetter < (int)y.root.smallestLetter)
            {
                tree.root.smallestLetter=x.root.smallestLetter;
            }else
            if((int)x.root.smallestLetter > (int)y.root.smallestLetter)
            {
                tree.root.smallestLetter = y.root.smallestLetter;
            }
